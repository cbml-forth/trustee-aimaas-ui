...
// ---------------------- Configuration ----------------------
const URLS = [
  "https://aimaas.trustee-1.ics.forth.gr/",
];
const ITERATIONS = 10;
const HEADLESS = true; // headed windows for realism
...
(async () => {
  ...
  const browsers = {
    ...
    WebKit: { type: webkit, launch: { headless: HEADLESS } },
  };

  for (const [browserName, cfg] of Object.entries(browsers)) {
    results[browserName] = {};

    for (const jsOn of [true]) {
  ...
  for (const [browser, jsBuckets] of Object.entries(data)) {
    for (const [jsLabel, sites] of Object.entries(jsBuckets)) {
      for (const [url, cacheBuckets] of Object.entries(sites)) {
        const coldFCPs = cacheBuckets.cold.map((r) => r.fcp).filter((v) =>
          v != null
        );
        const warmFCPs = cacheBuckets.warm.map((r) => r.fcp).filter((v) =>
          v != null
        );
